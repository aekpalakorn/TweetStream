...
public class RTProcess2 {
	...
	private static final double THRESHOLD_D_A = 10 * 2.0; // is the same as
															// before 2.0
	...
	public RTProcess2() {
		// Aek: For testing and preventing the out-of-memory exception,
		// disabling sketchQueue
	...
	/**
	 * Aek: Comment Read the data from tokenizedstream table and update the
	 * sketch tokenizedstream's schema: - t = timestamp (Y-m-d H:m:s) -
	 * status_ID - user_ID - tweet (["foo","bar",...])
	 * 
	 * Testing: - Disabling sketchQueue and any variables or methods depending
	 * on it - Disabling MySQL operations - Disabling speedlog and dspeedlog
	 * since they depend on sketchQueue
	 * 
	 * Refactoring: - For a continuous real-time monitoring, no need to set the
	...
	public void runTime(Timestamp dt)
		...
		// speedlog = new BufferedWriter(new FileWriter(OutputPath +
		// "/speedlog.txt"));
		// dspeedlog = new BufferedWriter(new FileWriter(OutputPath +
		// "/dspeedlog.txt"));
		...
		System.out.print("Fetching tweets from the API...");
		...
		List<JSONObject> twList = null;
		
		try {
			fetcher = new RealtimeAPITweetFetcher(uri, q, delayMin, intervalMin);
			twList = fetcher.fetch(new ArrayList<JSONObject>(), pageNo);
			System.out.println(twList.size()+" tweets retrieved.");
		} catch (Exception e) {
			e.printStackTrace();
		}
		// End getting tweets from the REST API
		
		/*
		 * Aek: Iterate over the current set of tweets
		 */
		for (JSONObject js: twList) {
			// / debug
			...
			try {
				...
				t = new Timestamp(utcDf.parse(js.getString("publishedTimeGMTStr")).getTime());
				String tweet = js.getString("content");
				String[] twTokens = tweet.split(" ");
				terms = new LinkedList<String>();
				
				for (int k = 0; k < twTokens.length; k++) {
					terms.add(twTokens[k]);
				}
			...
			// Aek: Testing
			System.out.println("activeTerms' size = "+activeTerms.activeTerms().size());
				...
				pool.execute(new Runnable() {
					...
					public void run() {
						for (Map.Entry<Integer, Integer> entry : f_counter
							...
							double ds = count / f_l;
							// currentSketch.firstOrderPulse(t, ds,
							// f_h, bucket);
						}
					...
					public void run() {
						for (Map.Entry<Integer, Integer> entry_i : f_counter
							...
							for (Map.Entry<Integer, Integer> entry_j : f_counter
								...
								ds /= f_l * (f_l - 1);
								// currentSketch.secondOrderPulse(t,
								// ds, f_h, bucket_i, bucket_j);
							}
						}
			...
			// ///// for difference
			if (t.after(DETECT_T)) {
				Timestamp oneday_before_t = new Timestamp(
						t.getTime() - CYCLE * 60 * 1000);

				int index = head;
				// Sketch sketch2 = sketchQueue[index %
				// MAX_QUEUE_SIZE];
				...
				try {
					...
					// final Pair pair =
					// estimator.zeroOrderDiff(currentSketch);
					// dspeedLogWrite(t, pair.v, pair.a);

					// if(t.after(one_min_after_lastTime) && pair.a
					// >= THRESHOLD_D_A && pair.v >= THRESHOLD_D_V){
					// saveSketch(currentSketch);
					// one_min_after_lastTime = new
					// Timestamp(t.getTime() + 60000);
					// }
				} catch (Exception e) {
					e.printStackTrace();
				}
	...
	private void checkFirstOrder() {
...

		{
			...
			for (int h = 0; h < H; h++) {
				...
				if (Math.abs(C_V - pair0.v) > 1e-9)
					System.out.println("V Error!" + h + " " + (C_V - pair0.v)
							+ " " + pair0.v);
				if (Math.abs(C_A - pair0.a) > 1e-9)
					System.out.println("A Error!" + h + " " + (C_A - pair0.a)
							+ " " + pair0.a);
			}
		}
	...
	private void saveSketch(Sketch sketch) throws Exception {
		...
		dir = dir.replace(" ", "_").replace("-", "_").replace(":", "_")
				.replace(".", "_");
		...
		try {
			...
			for (int h = 0; h < H; h++) {
				out = new BufferedWriter(new FileWriter(dir + "/firstOrderA_"
						+ h + ".txt"));
				out.write(firstOrderA[h]);
				out.close();

				out = new BufferedWriter(new FileWriter(dir + "/firstOrderV_"
						+ h + ".txt"));
				out.write(firstOrderV[h]);
				out.close();

				out = new BufferedWriter(new FileWriter(dir + "/secondOrderA_"
						+ h + ".txt"));
				out.write(secondOrderA[h]);
				out.close();

				out = new BufferedWriter(new FileWriter(dir + "/secondOrderV_"
						+ h + ".txt"));
	...
	private void saveSketchDiff(String dir, Sketch sketch) throws Exception {
		...
		try {
			BufferedWriter out = new BufferedWriter(new FileWriter(dir
					+ "/diff_zeroOrder.txt"));
			out.write(OutputSketch.outputZeroOrder(estimator
					.zeroOrderDiff(sketch)));
			out.close();

			String[] firstOrderA = OutputSketch.outputFirstOrderA(estimator
					.firstOrderDiff(sketch));
			String[] firstOrderV = OutputSketch.outputFirstOrderV(estimator
					.firstOrderDiff(sketch));
			String[] secondOrderA = OutputSketch.outputSecondOrderA(estimator
					.secondOrderDiff(sketch));
			String[] secondOrderV = OutputSketch.outputSecondOrderV(estimator
					.secondOrderDiff(sketch));
			for (int h = 0; h < H; h++) {
				out = new BufferedWriter(new FileWriter(dir
						+ "/diff_firstOrderA_" + h + ".txt"));
				out.write(firstOrderA[h]);
				out.close();

				out = new BufferedWriter(new FileWriter(dir
						+ "/diff_firstOrderV_" + h + ".txt"));
				out.write(firstOrderV[h]);
				out.close();

				out = new BufferedWriter(new FileWriter(dir
						+ "/diff_secondOrderA_" + h + ".txt"));
				out.write(secondOrderA[h]);
				out.close();

				out = new BufferedWriter(new FileWriter(dir
						+ "/diff_secondOrderV_" + h + ".txt"));
				out.write(secondOrderV[h]);
				out.close();
	...
	private void trackFirstOrder(String dir) {
		...
		for (int h = 0; h < H; h++)
			try {
				out_V = new BufferedWriter(new FileWriter(dir + "/trace_V_" + h
						+ ".txt"));
				out_A = new BufferedWriter(new FileWriter(dir + "/trace_A_" + h
						+ ".txt"));
